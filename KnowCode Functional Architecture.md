...
| **6. Retrieval** | [SearchEngine](file:///Users/deepg/Desktop/KnowCode/src/knowcode/retrieval/search_engine.py) / [RetrievalOrchestrator](file:///Users/deepg/Desktop/KnowCode/src/knowcode/retrieval/orchestrator.py) | **Hybrid search** (BM25 + FAISS fused via **Reciprocal Rank Fusion** with k=60, α=0.2 dense weight → sparse-heavy, configurable via `config.hybrid_alpha`) → **cross-encoder reranking** (VoyageAI `rerank-2.5`) → **dependency expansion** from graph → **context synthesis** with token budget + priority ranking |

### Key Retrieval Mechanics

- **HybridIndex** ([hybrid_index.py](file:///Users/deepg/Desktop/KnowCode/src/knowcode/retrieval/hybrid_index.py)): Runs BM25 sparse and FAISS dense searches in parallel, merges with **Reciprocal Rank Fusion** (RRF). The default `hybrid_alpha=0.2` is sparse-heavy (dense gets 20%, sparse 80%); set `config.hybrid_alpha: 0.5` for equal weighting.
