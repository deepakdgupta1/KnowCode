...
def main():
    ...
    for fn in ["src/knowcode/parsers/base.py", "src/knowcode/parsers/javascript_parser.py", "src/knowcode/parsers/java_parser.py"]:
        if Path(fn).exists():
            content = Path(fn).read_text()
            content = content.replace("def _get_text(self, node: Any, source_bytes: bytes) -> str:", "def _get_text(self, node: Any) -> str:")
            content = content.replace("_get_text(node, None)", "_get_text(node)")
            content = content.replace("_get_text(source_node, None)", "_get_text(source_node)")
            content = content.replace("_get_text(name_node, None)", "_get_text(name_node)")
            content = content.replace("_get_text(extends_node, None)", "_get_text(extends_node)")
            content = content.replace("_get_text(func_node, None)", "_get_text(func_node)")
            content = content.replace("_get_text(first_arg, None)", "_get_text(first_arg)")
            content = content.replace("_get_text(superclass_node, None)", "_get_text(superclass_node)")
            content = content.replace("_get_text(object_node, None)", "_get_text(object_node)")
            content = content.replace("_get_text(type_node, None)", "_get_text(type_node)")
    ...
    for filepath, file_errors in errors_by_file.items():
        ...
        for lineno, msg in sorted(file_errors, key=lambda x: x[0], reverse=True):
            ...
            if "missing a type annotation" in msg or "missing a return type annotation" in msg:
