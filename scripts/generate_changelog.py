...
def generate_entry(commits, range_str, origin="generated", summary=None):
    """Generates a formatted changelog entry by grouping the provided commits by their type.

    Args:
        commits: A list of commits to include in the entry.
        range_str: The version or commit range the entry covers.
        origin: The source tag for the entry. Defaults to "generated".
        summary: An optional overall summary for the entry. Defaults to None.

    Returns:
        A formatted string representing the changelog entry, or an empty string if no commits are provided.
    """
    if not commits:
        return ""
...
def main():
    """Parses command-line arguments and generates a changelog entry from git history into CHANGELOG.md."""
    ...
    new_entry = generate_entry(
        parsed_commits, range_str, origin=args.origin, summary=summary
    )
