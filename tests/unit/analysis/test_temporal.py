...
def test_temporal_analysis(git_repo) -> None:  # type: ignore
    ...
    authored_rels = [
        r for r in result.relationships if r.kind == RelationshipKind.AUTHORED
    ]
    ...
    modified_rels = [
        r for r in result.relationships if r.kind == RelationshipKind.MODIFIED
    ]
    ...
    changed_by_rels = [
        r for r in result.relationships if r.kind == RelationshipKind.CHANGED_BY
    ]
