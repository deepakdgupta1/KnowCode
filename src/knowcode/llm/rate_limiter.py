...
class RateLimiter:
    ...
    def __init__(
        self, persistence_path: Path = Path.home() / ".knowcode" / "usage_stats.json"
    ):
    ...
    def _load(self) -> None:
        ...
        if self.persistence_path.exists():
            try:
                ...
                print(
                    f"Warning: Failed to load usage stats, starting fresh. Error: {e}"
                )
                self.usage_data = {}
        else:
            self.usage_data = {}
    ...
    def check_availability(self, model_config: ModelConfig) -> bool:
        ...
        if len(last_minute_requests) >= model_config.rpm_free_tier_limit:
            print(
                f"  ⚠️ Limit Reached: {model_config.name} used {len(last_minute_requests)}/{model_config.rpm_free_tier_limit} RPM."
            )
            return False
        ...
        if len(last_day_requests) >= model_config.rpd_free_tier_limit:
            print(
                f"  ⚠️ Limit Reached: {model_config.name} used {len(last_day_requests)}/{model_config.rpd_free_tier_limit} RPD."
            )
            return False
