...
EXPLAIN_PATTERNS = [
    (
        re.compile(
            r"\b(explain|how\s+does|how\s+do|walk\s+through|step.by.step|describe|what\s+happens)\b",
            re.I,
        ),
        10,
    ),
...
DEBUG_PATTERNS = [
    (
        re.compile(
            r"\b(debug|bug|error|exception|fail|crash|broken|issue|problem)\b", re.I
        ),
        10,
    ),
    (
        re.compile(
            r"\b(why\s+(is|does|did|are)|what\'?s?\s+wrong|not\s+working|causes?)\b",
            re.I,
        ),
        8,
    ),
...
EXTEND_PATTERNS = [
    (
        re.compile(
            r"\b(add|implement|create|build|extend|modify|change|update)\b", re.I
        ),
        8,
    ),
...
def classify_query(query: str) -> Tuple[TaskType, float]:
    ...
    base_confidence = min(
        1.0, max_score / (total_possible * 0.5)
    )  # 50% of max = full confidence
