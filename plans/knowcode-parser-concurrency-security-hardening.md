...
### [x] Step 21 — Make proxy trust explicit and rate limiting enforceable
...
### [x] Step 22 — Run end-to-end adversarial, concurrency, and release gates
...
| 21 | `[x]` | `codex/hardening-s21-rate-limit` | 2026-08-14 | 2026-08-14 | Red phase reproduced both reviewed defects against production code with no fault injection: through `create_app` with the limiter **enabled**, `POST /api/v1/context/query` returned `500` (`assert 500 != 500`) because SlowAPI's `@limiter.limit` keys on the parameter *named* `request`, which held the pydantic `QueryRequest` body, not a Starlette `Request`; and Uvicorn's default `ProxyHeadersMiddleware` (`proxy_headers=True`, localhost trusted) rewrote `scope["client"]` from a spoofed `X-Forwarded-For` (`9.9.9.9` → `8.8.8.8`), rotating the IP-keyed bucket. 7 of the 8 new integration cases failed on `ImportError: cannot import name 'build_server'` (the hardened server entry did not exist). Green phase: 8 integration (real loopback socket, limiter enabled) + 4 server-hardening unit tests pass; focused `tests/unit/api` (46) and the plan's focused suite (`tests/unit/api` + `test_rate_limit_server.py` + `test_server_refinement.py`, 61) pass; 1486 global tests pass; five consecutive integration runs with no flake. Changed-module coverage: `main.py` 100%, `rate_limit.py` 100%, `api.py` 92% (residual misses are pre-existing untested endpoint branches, not changed lines) — 94% across the three. A negative control starts an intentionally trusting server and shows the same spoofing *does* rotate buckets (0×429), so the secure assertions are non-vacuous. Mutation check: re-enabling proxy trust (`proxy_headers=True`, trust `127.0.0.1`) fails the `X-Forwarded-For` security test and both proxy-config tests (the `Forwarded` test stays green because Uvicorn's middleware only reads `X-Forwarded-For`); reverting the endpoint rename fails the reachability test (red-phase `assert 500 != 500`). Ruff, mypy (91 files), strict MkDocs, and `git diff --check` pass. | Branched from `main` at `cba5c0e` (Step 20 merged). **No artifact format changes and no schema migration, so no rebuild is required.** ADR 6 implemented: `build_server()` is the single server constructor for both `start_server` and the rate-limit suite, and it sets `proxy_headers=False` with `forwarded_allow_ips=[]` — empty, not `None`, so Uvicorn cannot fall back to trusting localhost. `X-Forwarded-For`/`Forwarded` are ordinary untrusted headers in direct mode. Proxied deployment stays **unsupported**: no trusted-proxy CLI option is added; a future proxy mode must take an explicit narrow IP/CIDR allowlist and reject wildcard trust. Fix for the unreachable endpoint noted in Step 18's row: `query_context` now takes `request: Request` (the reserved SlowAPI name/type) and the body as `payload: QueryRequest`; FastAPI resolves the body by type, so the JSON request/response contract is unchanged. Residual local DoS boundary documented in `docs/architecture/hardening-contracts.md` and `docs/diagrams/README.md`: in direct mode every client is `127.0.0.1` and shares one bucket, so the per-tier limits (60/min standard, 10/min expensive) already cap a runaway local agent; no process-global bucket or API token is added. Step 21 owns proxy trust and limiter enforcement only; Step 22 exercises the assembled system under hostile inputs. |
| 22 | `[x]` | `codex/hardening-s22-release-gate` | 2026-08-14 | 2026-08-14 | Tests/docs/readiness only, no `src/` change. A new runtime-built adversarial repository (`tests/helpers/adversarial_repo.py`) carries every C2 construct, a quoted/Unicode/apostrophe path, hostile injection comments, and a secret-shaped token, and is driven through the assembled pipeline for **both** backends. New suites (30 cases, all green, five consecutive focused runs with no flake): `test_release_gate_pipeline.py` (16) — merged-graph endpoint integrity + exact C2 extraction (JS `inherits`, Rust `implements`, Python scope-confined calls, TS exported forms, Vue bindings, decorator-inclusive location), scan-order independence, coherent full generation with digest/checksum/count parity, the apostrophe path round-tripping safely through both vector stores, a watch add/modify/duplicate/delete/move batch landing on a doctor-accepted generation, a pre-publication failure preserving the prior generation, and 8 concurrent readers never seeing a mixed generation; `test_release_gate_security.py` (7) — the production `retrieve_context_for_query → build_prompt_request` path keeps retrieved content in the untrusted channel as one escaped line, the repo's exact injection markers cannot reach the instruction channel, and a repository-sourced secret/marker/query is never persisted by telemetry; `test_release_gate_soak.py` (2) — a seeded (`KNOWCODE_GATE_SEED`) bounded add/modify/delete/move soak under 6 concurrent readers, event-synchronized via `worker.drain()` (no timing-only sleeps), printing seed+op-log on failure; `test_release_gate_limitations.py` (5) — pins the four documented limitations. Global: 1516 tests pass on repeated full runs; Ruff, mypy (91 src files, unchanged), strict MkDocs, and `git diff --check` pass. | Branched from `main` at `dd3ec9e` with Step 21 merged. No production behavior changed, so no rebuild is required. The four Step 22-flagged discoveries are **documented as known limitations for this release and pinned by tests** in `test_release_gate_limitations.py` and `docs/architecture/hardening-contracts.md` (release-gate section) + `docs/RELEASE_CHECKLIST.md` (rewritten): (1) directory-level watch events are ignored — a rebuild re-keys the subtree; (2) an empty LanceDB index is never written as a broken artifact — the `_has_index_state` caller guard keeps the Step 12 backend residual unreachable; (3) a watched edit refreshes retrieval but not `knowledge.db` — doctor warns `store_stale_source_changed`, a rebuild refreshes the graph; (4) the prompt boundary's instruction/envelope overhead is bounded, declared, and accepted. Each fix is separately scoped product work, not patched in this tests-and-docs step. Independent-review substitute: self-review under a security/correctness lens found no critical/high issue; one **pre-existing** intermittent flake was observed and logged below (`test_lifespan_shutdown_reports_incomplete_work`), unrelated to this change. |
...
| 2026-08-14 | 21 | Discovery | `POST /api/v1/context/query` returned `500` on every call once the limiter was enabled — the reason Step 18's row flagged it unreachable. SlowAPI's `@limiter.limit` decorator locates the connection by the parameter *named* `request` and asserts it is a `starlette.requests.Request`; the endpoint's `request` parameter was the pydantic `QueryRequest` body, so the assertion always raised. The existing API tests missed it because every one of them disables the limiter. | Fixed by naming the Starlette parameter `request` (the reserved name/type) and the body `payload`; FastAPI resolves the body by type, so the JSON contract is unchanged. This is why Step 21's suite must run with the limiter **enabled** through the real stack — a disabled decorator hides both this defect and the proxy bypass. Pinned by `test_context_query_reachable_under_enabled_limiter`. |
| 2026-08-14 | 21 | Decision | Proxy trust is closed by two independent settings, not one: `proxy_headers=False` removes Uvicorn's `ProxyHeadersMiddleware` entirely, and `forwarded_allow_ips=[]` (empty, never `None`) means that even if proxy processing were re-enabled the trusted set is empty rather than the `127.0.0.1` fallback `None` resolves to. The mutation check confirmed a single flip is insufficient to bypass: only `proxy_headers=True` **and** a non-empty trusted list rotates the bucket. | Defense in depth for ADR 6's direct-server rule. No trusted-proxy option is exposed because KnowCode does not support proxied deployment; a future proxy mode is a separate design that must take an explicit narrow IP/CIDR allowlist and reject wildcard trust. The residual local-DoS boundary is documented rather than eliminated: in direct mode all clients are `127.0.0.1` and share one bucket, so the per-tier limits already cap a runaway local agent. Pinned by `test_spoofed_x_forwarded_for_shares_one_bucket` and its negative control. |
| 2026-08-14 | 22 | Decision | The gate proves the *assembled* system on one adversarial repository rather than re-testing primitives. `tests/helpers/adversarial_repo.py` builds the tree at runtime (never committed, so no exotic filename is ever checked out) with every C2 construct plus three hostile inputs: a quoted/Unicode/apostrophe path, injection comments, and a secret-shaped token. The apostrophe is the pointed adversarial property — `normalize_file_identity` preserves it verbatim, so it enters the entity id and chunk id and must be neutralized by the Step 12 LanceDB digest predicate, which the gate confirms end to end for both backends. | Closes the traceability table's "Final proof: 22" column with automated evidence over hostile input rather than the trivial trees the per-step suites used. The shared builder and a `graph_gate.assert_no_dangling_endpoints` helper live in `tests/helpers/` (the one shared test package); the four gate suites import them. Rate limiting through the real socket with spoofed headers is already the Step 21 integration suite and is referenced, not duplicated. |
| 2026-08-14 | 22 | Decision | The four Step 22-flagged discoveries are **resolved as documented limitations for this release, each pinned by a test**, not patched in this tests-and-docs step: (1) directory-level watch events ignored — `IndexingHandler` returns on `is_directory`, a rebuild re-keys the subtree; (2) empty-LanceDB save residual — the `_has_index_state` caller guard keeps `flush()` from ever writing the broken artifact; (3) a watched edit refreshes retrieval but not `knowledge.db` — retrieval stays fresh, graph `search` goes stale, `knowcode doctor` warns `store_stale_source_changed`, a rebuild refreshes it; (4) prompt/envelope overhead is bounded, declared, and accepted. | The plan's rollback rule requires product defects found here to become new scoped steps rather than weakened assertions. None of these is a regression, so each is documented (`docs/architecture/hardening-contracts.md` release-gate section + rewritten `docs/RELEASE_CHECKLIST.md`) and pinned in `test_release_gate_limitations.py`; item (3)'s remedy — incremental `GraphBuilder` for watched edits — is roadmap product work, and if any is later *fixed* the pinning test and checklist move with it. |
| 2026-08-14 | 22 | Discovery | `tests/unit/api/test_app_lifespan.py::test_lifespan_shutdown_reports_incomplete_work` is a **pre-existing** intermittent flake: it uses a `shutdown_timeout=0.2` wall-clock deadline plus a `gate.wait(timeout=5.0)` as its correctness mechanism, so under full-suite load the worker's `_queue.join(0.2)` can time out (`completed=False`) at the same instant its `in_flight`/`pending` snapshot goes empty (`incomplete_work=()`), failing `assert report.incomplete_work`. Observed once in three full runs; passes 5/5 in isolation and 3/3 alongside the new gate files. | Not caused by Step 22 (tests/docs only; the gate suites pass deterministically) and out of its scope by anti-pattern 18 (no unrelated test fix in this PR). It violates the global rule against timing-only concurrency synchronization; logged for a maintenance change that replaces the deadline with a barrier/event so the assertion observes the blocked worker deterministically. |
