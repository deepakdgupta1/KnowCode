...
| 2026-08-13 | 04 | Discovery | `VueParser._extract_entities`, `_parse_script_content`, and `_parse_template_content` are unreachable tree-sitter-mode scaffolding: `VueParser` overrides `parse_file` and holds no Vue grammar. | Left in place to keep the Step 04 PR to one logical change. Remove during Step 05 or Step 07, when Vue parsing is revisited; it is ~35 lines of the file's remaining uncovered code. |
